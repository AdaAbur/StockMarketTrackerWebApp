document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadStockNews(panel, sym) {
    try {
      const items = await fetchCompanyNews(sym);
      panel.innerHTML = "";
      if (items.length === 0) {
        const empty = document.createElement("p");
        empty.className = "stock-news-loading";
      ...
      items.forEach((item) => {
        ...
        headline.className = "stock-news-headline";
        headline.textContent = item.headline;

        const source = document.createElement("span");
        source.className = "stock-news-source";
      ...
      err.className = "stock-news-loading";
  ...
  headerMain.className = "stock-header-main";
  ...
  newsPanel.className = "stock-header-news";
  const newsLoading = document.createElement("p");
  newsLoading.className = "stock-news-loading";
  newsLoading.textContent = "Loading news...";
  newsPanel.appendChild(newsLoading);
  header.appendChild(newsPanel);

  loadStockNews(newsPanel, stock.symbol);
